            ...
            switch (type) {
                case 'matrix':
                    if ($("id_" + data).down('.form-radio')) {
                        $$('input[id^="input_'+data+subField+'_"]:not([id*="_disabled"]').each(function (radio) {
                            if (radio.checked) {
            ...
            case "html":
                try {
                    ...
                    if (spans.length == 0) {
                        ...
                        //get subLabel and check
                        if (subLabel) {
                            var subLabelContent = subLabel[0].innerHTML;
                            if (subLabelContent.indexOf(calc.replaceText) > -1) {
                              subLabelContent = subLabelContent.replace(re, '<span class="replaceTag ' + result + "_" + calc.replaceText + '" default="'+def+'">' + output + '</span>');
                              subLabel[0].update(subLabelContent);
                            }
                        }
                    } else {
                        ...
                        subLabel.each(function (subl){
                            var re = new RegExp("\{" + calc.replaceText + "\}", "g");
                            var def = calc.defaultValue || "";
                            var content = subl.innerHTML.replace(re, '<span class="replaceTag ' + result + "_" + calc.replaceText + '" default="'+def+'">' + output + '</span>');
                            subl.update(content);
                        });
                    }
                } catch (e) {
